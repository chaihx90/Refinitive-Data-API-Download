...
# Key in credentials to Refinitiv Datascope below!
myUsername = ""
myPassword = ""
...
EndPointURL='https://selectapi.datascope.refinitiv.com/RestApi/v1/StandardExtractions/PackageDeliveries'
...
#print(filePath + "\\Data\\orderbook.csv.gz")
try:
    temp_df =  pd.read_csv(filePath + "\\Data\\orderbook.csv.gz")
...
try:
    temp_df =  pd.read_csv(filePath + "\\Data\\transaction.csv.gz")
    temp_df = pd.to_datetime(temp_df['Date-Time'])
    date_df = pd.DataFrame(temp_df.dt.date.astype(str).unique())
    trans_exist = True
except:
...
if not ob_df.empty:
    ...
    ob_consol_name = filePath + "\\Data\\orderbook.csv.gz"
    if ob_exist:
...
if not ob_df.empty:
    ...
    trans_consol_name = filePath + "\\Data\\transaction.csv.gz"
    if trans_exist:
